...
def kartendaten(hass: HomeAssistant) -> list[dict[str, Any]]:
    """Alle Anlagen mit ihrem Schaubild, frisch aus der Registrierung."""
    return schaubild_daten(_anlagen(hass))


@websocket_api.websocket_command({vol.Required("type"): f"{DOMAIN}/schaubild"})
@callback
def _ws_schaubild(hass: HomeAssistant, connection, msg: dict[str, Any]) -> None:
    """Die Schaubilder aller Anlagen zurückgeben."""
    connection.send_result(msg["id"], kartendaten(hass))
